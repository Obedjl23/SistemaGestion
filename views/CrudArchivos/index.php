...
if ($id_empresa['id_empresa'] == 1) { //admin
  ...
  $sel = $con->query("SELECT * FROM archivo WHERE id_empresa = '$id_empresa'");
  while ($row = $sel->fetch_assoc()) {
    $tmp = $row;
    array_push($res, $tmp);
  }
              ...
              <table class="table mt-2">
                ...
                <tbody>
                 ...
                 <?php } ?>
                      ...
                      <td>
                        <a class="btn btn-primary" target="_black" href="<?php echo 'http://' . $_SERVER['HTTP_HOST'] . '/views/CrudArchivos/' . $val['url']; ?>">Ver Archivo pagina</a>

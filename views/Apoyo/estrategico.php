...
if ($id_empresa['id_empresa'] == 1) { //admin
  $sql = $con->query("SELECT per.id, per.title, per.description, per.url, emp.emp_nombre from apoyo per
  ...
  $sql = $con->query("SELECT * FROM apoyo WHERE id_empresa = '$id_empresa'");
  while ($row = $sql->fetch_assoc()) {
    $tmp = $row;
    array_push($res, $tmp);
  }

...
if ($id_empresa['id_empresa'] == 1) { //admin
  $sel = $con->query("SELECT per.id, per.title, per.description, per.url, emp.emp_nombre from operativo per
  ...
  $sel = $con->query("SELECT * FROM operativo WHERE id_empresa = '$id_empresa'");
  while ($row = $sel->fetch_assoc()) {
    $tmp = $row;
    array_push($res, $tmp);
  }
...
if ($id_empresa['id_empresa'] == 1) { //admin
  $sel = $con->query("SELECT pe.id, pe.title, pe.description, pe.url, em.emp_nombre from operativo pe
        ...
        <div class="container">
          <div class="row justify-content-md-center">
            <div class="col-md-auto">
              <h1 <i class="fa fa-align-center fw-bold" aria-hidden="true">PROCESOS - OPERATIVOS</i></h1>
